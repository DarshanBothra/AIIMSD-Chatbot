...
def fetchSymptoms(prompt: str)->list[str]:
    ...
    try:
        response = model.generate_content(f"what are all the symptoms from the following prompt, if the prompt mentions pain, include the areas of pain in the response as 'pain in and body part named' (just the words) else return None:{prompt}")
